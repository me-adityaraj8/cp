...
void solve() {
  ...
  map<ll, vll> mp;
  f(i, 0, n) mp[a[i]].pb(i);
  ll sum0 = 0, last_smallest = 0;
  bool flag = true;
  for (auto it = next(mp.begin()); it != mp.end(); it++) {
    ll sum1 = it->first - sum0;
    ll size = prev(it)->second.size();
    ll last = sum1 / size, rem = sum1 % size;
    if (rem > 0 || last <= last_smallest)
      flag = false;
    if (!flag)
      break;
    last_smallest = last;
    for (auto gt : prev(it)->second) {
      a[gt] = last;
    }
    sum0 = it->first;
  }
  if (!flag || (mp.begin()->first > 0)) {
    cout << -1 << endl;
    return;
  }
  auto it = prev(mp.end());
  for (auto ot : it->second) {
    a[ot] = last_smallest + 1;
  }
  cout << a;
  cout << endl;
}
